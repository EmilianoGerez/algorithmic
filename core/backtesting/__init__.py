...
from ..data.models import (
    Candle, MarketData, Signal, Position, BacktestResult,
    TimeFrame, SignalDirection, StrategyConfig
)
from ..data.feeds import BacktestDataFeed
from ..data.adapters import DataAdapter
from ..strategies.base_strategy import BaseStrategy
from ..risk import RiskManager, RiskLimits, FixedRiskPositionSizer
...
class CoreBacktestEngine(BacktestEngine):
    ...
    def run_backtest(
        ...
        position_sizer = FixedRiskPositionSizer(
            risk_per_trade=float(config.risk_limits.max_position_size) / 5  # Conservative sizing
        )
    ...
    def _on_candle(self, candle: Candle) -> None:
        ...
        market_data = {candle.timeframe: MarketData(
    ...
    def _process_signals(self, risk_manager: RiskManager, config: BacktestConfig) -> None:
        ...
        for signal in self.signals:
            if signal.signal_type.value == "entry":
                ...
                if assessment['approved']:
                    ...
                    adjusted_price = self._apply_costs(signal.entry_price, config, "entry")
    ...
    def _apply_costs(self, price: Decimal, config: BacktestConfig, action: str) -> Decimal:
    ...
    def _close_all_positions(self, risk_manager: RiskManager, config: BacktestConfig) -> None:
        ...
        for position in self.positions[:]:  # Copy list to avoid modification during iteration
            if position.symbol in self.current_prices:
                exit_price = self._apply_costs(
                    self.current_prices[position.symbol],
                    config,
                    "exit"
                )
    ...
    def _generate_results(
        ...
        return BacktestResult(
            ...
            losing_trades=risk_manager.metrics.total_trades - risk_manager.metrics.winning_trades,
...
class OptimizationEngine:
    ...
    def optimize_parameters(
        ...
        for params in combinations:
            try:
                ...
                # Run backtest
                result = self.backtest_engine.run_backtest(strategy, market_data, config)
    ...
    def _calculate_objective_score(self, result: BacktestResult, objective: str) -> float:
        ...
        for symbol in symbols:
            try:
                result = self.run_single_backtest(
                    strategy, symbol, timeframe, start_date, end_date, initial_capital
                )
