...
class LiveDataFeed(DataFeed):
    ...
    def __init__(self, adapter: DataAdapter, symbols: List[str], timeframes: List[TimeFrame]):
            ...
            try:
                for symbol in self.symbols:
                    for timeframe in self.timeframes:
                        # Get latest candle
                        latest_candle = self.adapter.get_latest_candle(symbol, timeframe)
                        ...
                        if latest_candle:
                            ...
                            if not last_timestamp or latest_candle.timestamp > last_timestamp:
...
class BacktestDataFeed(DataFeed):
    ...
    def start(self) -> None:
        ...
        print(f"Backtest data feed started with {len(self.market_data.candles)} candles")
