...
class TechnicalIndicators:
    ...
    @staticmethod
    def macd(candles: List[Candle], fast_period: int = 12, slow_period: int = 26, 
             signal_period: int = 9) -> Dict[str, List[IndicatorResult]]:
        """
        ...
        for i in range(start_idx, len(fast_ema)):
            ...
            macd_line.append(IndicatorResult(
                value=macd_value,
                timestamp=candles[i].timestamp,
        ...
        for i, macd_result in enumerate(macd_line):
            ...
            signal_line.append(IndicatorResult(
                value=signal_value,
        ...
        for i in range(len(signal_line)):
            ...
            histogram.append(IndicatorResult(
                value=hist_value,
    ...
    @staticmethod
    def bollinger_bands(candles: List[Candle], period: int = 20, 
                       std_dev: float = 2.0) -> Dict[str, List[IndicatorResult]]:
        """
        Bollinger Bands
        ...
        for i in range(period - 1, len(candles)):
            ...
            upper_band.append(IndicatorResult(value=upper, timestamp=timestamp, metadata=metadata))
            middle_band.append(IndicatorResult(value=sma, timestamp=timestamp, metadata=metadata))
            lower_band.append(IndicatorResult(value=lower, timestamp=timestamp, metadata=metadata))
    ...
    def stochastic(candles: List[Candle], k_period: int = 14, 
                   d_period: int = 3) -> Dict[str, List[IndicatorResult]]:
        """
        ...
        for i in range(k_period - 1, len(candles)):
            ...
            k_values.append(IndicatorResult(
                value=k_value,
                timestamp=candles[i].timestamp,
        ...
        if len(k_values) >= d_period:
            for i in range(d_period - 1, len(k_values)):
                d_value = sum(k_values[j].value for j in range(i - d_period + 1, i + 1)) / d_period
                d_values.append(IndicatorResult(
                    value=d_value,
...
class EMASystem:
    ...
    def __init__(self, fast_period: int = 9, medium_period: int = 20, slow_period: int = 50):
        """
    ...
    def check_ema_alignment(self, emas: Dict[str, List[IndicatorResult]], 
                           direction: str, index: int = -1) -> bool:
        """
    ...
    def get_trend_strength(self, emas: Dict[str, List[IndicatorResult]], 
                          index: int = -1) -> float:
        """
    ...
    def check_consecutive_closes(self, candles: List[Candle], emas: Dict[str, List[IndicatorResult]], 
                               direction: str, consecutive_count: int = 2) -> bool:
        """
