...
from ..data.models import (
    Candle, Signal, SignalDirection, SignalType, 
    TimeFrame, MarketData, FVGZone, StrategyConfig
)
...
class SignalProcessor:
    ...
    def _create_entry_signal(self, fvg_zone: FVGZone, context: SignalContext, 
    ...
    def _calculate_stop_and_target(self, fvg_zone: FVGZone, entry_price: Decimal, 
                                 candles: List[Candle]) -> Tuple[Optional[Decimal], Optional[Decimal]]:
    ...
    def _find_swing_levels(self, candles: List[Candle], lookback: int = 20) -> Dict[str, Decimal]:
    ...
    def _filter_fvgs_by_quality(self, fvg_zones: List[FVGZone], 
                               context: SignalContext) -> List[FVGZone]:
    ...
    def _validate_signals(self, signals: List[Signal], context: SignalContext) -> List[Signal]:
        """Validate and filter signals"""
        validated_signals = []
    ...
    def _is_too_close_to_previous_signal(self, signal: Signal, min_gap_minutes: int = 30) -> bool:
    ...
    def get_signal_statistics(self) -> Dict[str, any]:
        ...
        long_signals = sum(1 for s in self.generated_signals if s.direction == SignalDirection.LONG)
...
class MultiTimeframeEngine:
    ...
    def generate_signals(self) -> List[Signal]:
        ...
        ltf_timeframe = min(self.timeframe_data.keys(), key=lambda x: self._timeframe_to_minutes(x))
        ltf_data = self.timeframe_data[ltf_timeframe]
        
        # Higher timeframes for context
        htf_data = {
            tf: data for tf, data in self.timeframe_data.items()
            if tf != ltf_timeframe
        }
