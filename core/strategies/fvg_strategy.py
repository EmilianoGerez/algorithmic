...
class FVGStrategy(BaseStrategy):
    ...
    def generate_signals(
        ...
        if not emas or not all(
            len(emas[key]) > 0 for key in ["fast", "medium", "slow"]
        ):
            return []
    ...
    def _check_entry_signal(
        ...
        if not self.ema_system.check_consecutive_closes(
            ltf_data.candles,
            emas,
            fvg_zone.direction.value,
            self.consecutive_closes,
        ):
            return None
        ...
        stop_loss, take_profit = self._calculate_stop_and_target(
            fvg_zone, current_candle, ltf_data.candles
        )
