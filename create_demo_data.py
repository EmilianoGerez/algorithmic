...
def create_mock_backtest_data(results_dir: Path, data_file: str):
    ...
    for i in range(50, len(market_df), np.random.randint(40, 100)):
        ...
        events.append({
            ...
            'high': market_df.iloc[i]['high'] if pivot_type == 'high' else market_df.iloc[i]['close'],
            'low': market_df.iloc[i]['low'] if pivot_type == 'low' else market_df.iloc[i]['close'],
            'price': market_df.iloc[i]['high'] if pivot_type == 'high' else market_df.iloc[i]['low']
        })
...
""")
    ...
    print(f"✅ Created config.yaml and provenance.json")
...
def main():
    parser = argparse.ArgumentParser(description="Create mock backtest data for visualization testing")
    ...
    if success:
        ...
        print(f"from quant_algo.visual.plot_builder import build_plotly_from_data")
        ...
        print(f"fig = build_plotly_from_data(data_df, trades_df, events_df)")
        print(f"fig.show()\"")
    else:
