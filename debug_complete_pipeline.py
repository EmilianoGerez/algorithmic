...
def debug_complete_pipeline():
    ...
    fvg_detector = FVGDetector(
        "240", min_gap_atr=0.05, min_gap_pct=0.01, min_rel_vol=0.0
    )
    ...
    for i, row in df.iterrows():
        ...
        candle = Candle(
            ts=row["timestamp"].tz_localize(UTC)
            if row["timestamp"].tz is None
            else row["timestamp"],
            open=row["open"],
            high=row["high"],
            low=row["low"],
        ...
        for h4_candle in completed_h4s:
            ...
            print(
                f"   OHLCV: {h4_candle.open:.2f}/{h4_candle.high:.2f}/{h4_candle.low:.2f}/{h4_candle.close:.2f}/{h4_candle.volume:.0f}"
            )
            ...
            for fvg_event in new_fvg_events:
                print(
                    f"   🎯 FVG DETECTED: {fvg_event.side} [{fvg_event.bottom:.2f}, {fvg_event.top:.2f}] strength={fvg_event.strength:.3f}"
                )
                ...
                if result.success and result.pool_created:
                    ...
                    if pool:
                        print(
                            f"      Pool details: TF={pool.timeframe} [{pool.bottom:.2f}, {pool.top:.2f}] strength={pool.strength:.3f}"
                        )
                else:
                    print(
                        f"   ❌ POOL CREATION FAILED: {result.reason if hasattr(result, 'reason') else 'Unknown'}"
                    )
        ...
        if pools_created:  # Only check if we have pools
            ...
            for pool_id in pools_created:
                ...
                if pool and pool.bottom <= current_price <= pool.top:
                    ...
                    print(
                        f"   🎯 ZONE TOUCH: {candle.ts} price={current_price:.2f} in pool {pool_id} [{pool.bottom:.2f}, {pool.top:.2f}]"
                    )
                    ...
                    if ema21 is not None and ema50 is not None:
                        print(
                            f"      EMA21={ema21:.2f}, EMA50={ema50:.2f}, Close={candle.close:.2f}"
                        )

                        # Check if close is above EMA21 (bullish signal condition)
                        if pool.timeframe == "240" and candle.close > ema21:
                            print(
                                f"      ✅ BULLISH SIGNAL CONDITION: Close {candle.close:.2f} > EMA21 {ema21:.2f}"
                            )
                            ...
                            print(
                                f"      Killzone check: {hour}:00 in_killzone={in_killzone}"
                            )

                            if in_killzone:
                                print("      ✅ IN KILLZONE - Signal candidate viable")
                                signal_candidates.append(
                                    (candle.ts, pool_id, "bullish")
                                )
                            else:
                                print("      ❌ OUTSIDE KILLZONE - Signal rejected")
                        else:
                            print(
                                f"      ❌ BEARISH CONDITION: Close {candle.close:.2f} <= EMA21 {ema21:.2f}"
                            )
                    else:
    ...
    if fvg_events:
        ...
        for i, event in enumerate(fvg_events):
            print(
                f"{i + 1}. {event.ts} {event.side} [{event.bottom:.2f}, {event.top:.2f}] strength={event.strength:.3f}"
            )
    ...
    target_candles = [
        c for c in candles if abs((c.ts - may20_14h).total_seconds()) < 3600
    ]  # ±1 hour
    ...
    if target_candles:
        ...
        for candle in target_candles[:5]:  # Show first 5
            print(
                f"  {candle.ts} OHLC: {candle.open:.2f}/{candle.high:.2f}/{candle.low:.2f}/{candle.close:.2f}"
            )
            ...
            for pool_id in pools_created:
                ...
                if pool and pool.bottom <= candle.close <= pool.top:
                    print(
                        f"    🎯 TOUCHES POOL {pool_id}: [{pool.bottom:.2f}, {pool.top:.2f}]"
                    )
