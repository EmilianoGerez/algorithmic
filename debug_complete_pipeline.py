...
from core.strategy.signal_candidate import SignalCandidateFSM, CandidateConfig
...
def debug_complete_pipeline():
    ...
    for i, row in df.iterrows():
        ...
        candle = Candle(
            ts=row['timestamp'].tz_localize(timezone.utc) if row['timestamp'].tz is None else row['timestamp'],
            open=row['open'],
            high=row['high'], 
            low=row['low'],
            close=row['close'],
                ...
                if pool and pool.bottom <= current_price <= pool.top:
                    ...
                    if ema21 is not None and ema50 is not None:
                        ...
                        if pool.timeframe == "240" and candle.close > ema21:
                            ...
                            if in_killzone:
                                print(f"      ✅ IN KILLZONE - Signal candidate viable")
                                signal_candidates.append((candle.ts, pool_id, "bullish"))
                            else:
                                print(f"      ❌ OUTSIDE KILLZONE - Signal rejected")
                        else:
    ...
    may20_14h = datetime(2024, 5, 20, 14, 0, tzinfo=timezone.utc)
