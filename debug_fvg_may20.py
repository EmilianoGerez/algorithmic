...
def analyze_may_20_fvgs():
    ...
    for fvg in focus_fvgs:
        print(f"  {fvg['time'].strftime('%H:%M')} | {fvg['type']:<8} | Gap: {fvg['gap_size']:.1f} ({fvg['gap_pct']:.3f}%) | Range: {fvg['lower']:.1f} - {fvg['upper']:.1f}")
    ...
    for fvg in sorted_fvgs:
        ...
        print(f"  {time_str} | {fvg['type']:<8} | Gap: {fvg['gap_size']:.1f} ({fvg['gap_pct']:.3f}%) | Range: {fvg['lower']:.1f} - {fvg['upper']:.1f}")
    
    # Analyze price movement around May 20, 14:00
    focus_df = df[(df['timestamp'] >= focus_start) & (df['timestamp'] <= focus_end)].copy()
    print(f"\n📈 Price action during focus window:")
    print(f"  Start: {focus_df.iloc[0]['close']:.1f} at {focus_df.iloc[0]['timestamp'].strftime('%H:%M')}")
    print(f"  End: {focus_df.iloc[-1]['close']:.1f} at {focus_df.iloc[-1]['timestamp'].strftime('%H:%M')}")
    ...
    for fvg in pre_fvgs:
        ...
        if fvg['type'] == 'bullish' and focus_low <= fvg['upper'] and focus_high >= fvg['lower']:
            touched_fvgs.append(fvg)
        elif fvg['type'] == 'bearish' and focus_low <= fvg['upper'] and focus_high >= fvg['lower']:
            touched_fvgs.append(fvg)
    
    print(f"  Found {len(touched_fvgs)} pre-existing FVGs that were touched during focus window:")
    for fvg in touched_fvgs:
        created_time = fvg['time'].strftime('%Y-%m-%d %H:%M')
        print(f"    {created_time} | {fvg['type']:<8} | Range: {fvg['lower']:.1f} - {fvg['upper']:.1f} | Gap: {fvg['gap_pct']:.3f}%")
...
def create_chart(df, fvgs, focus_start, focus_end):
    ...
    plt.plot(chart_df['timestamp'], chart_df['close'], 'b-', linewidth=1, label='Close Price')
    plt.fill_between(chart_df['timestamp'], chart_df['low'], chart_df['high'], alpha=0.3, color='lightblue')
    ...
    for fvg in chart_fvgs:
        ...
        alpha = 0.3
        plt.fill_between([fvg['time'] - pd.Timedelta(minutes=15), fvg['time'] + pd.Timedelta(minutes=15)], 
                        fvg['lower'], fvg['upper'], color=color, alpha=alpha, 
                        label=f"{fvg['type']} FVG" if fvg == chart_fvgs[0] else "")
    
    # Highlight focus window
    plt.axvspan(focus_start, focus_end, color='yellow', alpha=0.2, label='Focus Window (14:00-18:00)')
...
if __name__ == "__main__":
    try:
        ...
        if touched_fvgs:
            print(f"🎯 Key finding: {len(touched_fvgs)} pre-existing FVGs were touched during your target time window")
            print(f"    This suggests potential entry opportunities around May 20, 14:00-18:00")
        else:
            print(f"⚠️  No pre-existing FVGs were touched during the focus window")
            print(f"    Consider expanding the time window or looking at different timeframes")
