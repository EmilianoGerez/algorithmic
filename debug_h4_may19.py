...
def test_h4_fvg_may19():
    ...
    for i, (ts, row) in enumerate(df_h4.iterrows()):
        print(
            f"  {i}: {ts} | O:{row['open']:.2f} H:{row['high']:.2f} L:{row['low']:.2f} C:{row['close']:.2f} V:{row['volume']:.0f}"
        )
    ...
    for i, candle in enumerate(candles):
        ...
        print(
            f"  OHLCV: {candle.open:.2f}/{candle.high:.2f}/{candle.low:.2f}/{candle.close:.2f}/{candle.volume:.0f}"
        )
        ...
        for event in new_events:
            print(
                f"    FVG: {event.side} [{event.bottom:.2f}, {event.top:.2f}] strength={event.strength:.3f}"
            )
        ...
        if len(candles) >= 3:
            for i in range(2, len(candles)):
                ...
                if prev.high < next_candle.low:
                    ...
                    print(
                        f"  Potential bullish FVG at {next_candle.ts}: gap={gap_size:.2f} ({gap_pct * 100:.2f}%)"
                    )
                ...
                if prev.low > next_candle.high:
                    ...
                    gap_pct = gap_size / prev.close
                    print(
                        f"  Potential bearish FVG at {next_candle.ts}: gap={gap_size:.2f} ({gap_pct * 100:.2f}%)"
                    )
