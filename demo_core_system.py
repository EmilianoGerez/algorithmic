...
from core import (
    # Data models
    Candle, MarketData, TimeFrame, SignalDirection,
    
    # Strategy system
    FVGStrategy, create_fvg_strategy_config, strategy_registry,
    
    # Indicators
    FVGDetector, FVGFilterPresets, TechnicalIndicators,
...
def demonstrate_fvg_detection():
    ...
    for i, fvg in enumerate(fvg_zones[:5]):  # Show first 5
        print(f"  FVG {i+1}: {fvg.direction.value} | "
...
def demonstrate_strategy_system():
    ...
    print(f"  Required Timeframes: {[tf.value for tf in strategy.get_required_timeframes()]}")
