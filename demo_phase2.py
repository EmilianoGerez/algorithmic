...
def demo_acceptance_criteria() -> bool:
    ...
    for i, candle in enumerate(candles):
        ...
        if result:
            ...
            for h1_candle in result:
                print(
                    f"   H1: {h1_candle.ts} | O:{h1_candle.open:.2f} H:{h1_candle.high:.2f} L:{h1_candle.low:.2f} C:{h1_candle.close:.2f} V:{h1_candle.volume}"
                )

    print()
    print(f"✅ RESULT: {len(completed_h1_candles)} H1 candles completed")
    print(
        f"✅ ACCEPTANCE CRITERIA: {'PASSED' if len(completed_h1_candles) == 2 else 'FAILED'}"
    )
    print()
    ...
    if len(completed_h1_candles) >= 2:
        ...
        expected_1 = datetime(2024, 1, 1, 10, 0, 0, tzinfo=UTC)
        expected_2 = datetime(2024, 1, 1, 11, 0, 0, tzinfo=UTC)
...
def demo_multi_timeframe() -> bool:
    ...
    completed_counts = dict.fromkeys(multi_agg.timeframe_names, 0)
        ...
        for tf_name, tf_candles in results.items():
            if tf_candles:
                ...
                for tf_candle in tf_candles:
                    print(
                        f"✅ {tf_name} completed: {tf_candle.ts} | C:{tf_candle.close:.2f}"
                    )
...
def demo_performance() -> bool:
    ...
    aggregator = TimeAggregator(
        tf_minutes=60, buffer_size=100
    )  # Smaller buffer for stress test
    ...
    print("\n✅ Performance Results:")
    print("  Total candles processed: 500,000")
...
def demo_memory_efficiency() -> bool:
    ...
    for i in range(500):  # 500 minutes = 8+ hours
        ...
        if i % 100 == 99:
            ...
            print(
                f"  After {i + 1} candles: buffer size = {buffer_len} (limit: {buffer_size})"
            )

    final_buffer_size = len(aggregator._buffer)
    print(f"\n✅ Final buffer size: {final_buffer_size} (limit: {buffer_size})")
    print(
        f"✅ Memory bounded: {'PASSED' if final_buffer_size <= buffer_size else 'FAILED'}"
    )
