...
from core import (
    # Data models
    Candle, MarketData, Signal, TimeFrame, SignalDirection, SignalType,
    # Data integration
    DataAdapterFactory, BacktestDataFeed, MultiSymbolDataFeed,
    # Strategy system
    FVGStrategy, create_fvg_strategy_config,
    # Risk management
    RiskManager, RiskLimits, PositionSizer,
    # Backtesting
    BacktestRunner, BacktestConfig, CoreBacktestEngine
)
...
def demo_multi_symbol_feed():
    ...
    def on_candle_received(candle):
        ...
        if len(received_signals) <= 5:  # Show first 5 only
            print(f"📊 Received: {candle.symbol} - {candle.timestamp} - Close: ${candle.close}")
...
def demo_portfolio_management():
    ...
    for sizer_name, sizer in sizers:
        ...
        for signal in test_signals:
            ...
            print(f"   {signal.symbol}: {assessment['position_size']:.2f} shares (${assessment.get('risk_amount', 0):.2f} risk)")
...
def main():
    ...
    try:
        ...
        print("  ✅ Risk Management - Position sizing, portfolio limits, drawdown control")
        print("  ✅ Backtesting Engine - Comprehensive backtesting with risk integration")
