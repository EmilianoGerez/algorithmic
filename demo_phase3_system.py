...
class Phase3Demo:
    ...
    async def demo_signal_processing(self):
        ...
        for signal in test_signals:
            print(
                f"🎯 Processing signal: {signal.symbol} {signal.direction.value} @ ${signal.entry_price}"
            )
    ...
    async def demo_order_management(self):
        ...
        for position in positions:
            print(
                f"    {position.symbol}: {position.direction.value} {position.quantity} @ ${position.entry_price}"
            )
    ...
    def _on_streaming_data(self, candle):
        ...
        if len(self.demo_signals) < 3:
            print(
                f"  📊 Live data: {candle.symbol} - ${candle.close} @ {candle.timestamp.strftime('%H:%M:%S')}"
            )
