...
class BacktestDataExporter:
    ...
    def _init_trades_file(self) -> None:
        ...
        with open(self.trades_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                'id', 'symbol', 'side', 'entry_ts', 'exit_ts',
                'entry_price', 'exit_price', 'size', 'pnl', 'fees',
                'duration_minutes', 'exit_reason'
            ])
    
    def add_candle(self, timestamp: datetime, open_price: float, high: float, 
    ...
    def add_trade(self, trade_id: str, symbol: str, side: str,
                  entry_ts: datetime, exit_ts: datetime,
                  entry_price: float, exit_price: float,
                  size: float, pnl: float, fees: float = 0.0,
    ...
    def add_fvg_event(self, timestamp: datetime, top: float, bottom: float,
                      timeframe: str = "1m", strength: float = 1.0) -> None:
    ...
    def add_pivot_event(self, timestamp: datetime, price: float, side: str,
                        strength: float = 1.0, atr_distance: float = 0.0) -> None:
        """Add Pivot event.
