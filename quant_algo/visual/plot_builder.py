...
def build_plotly(run_ctx: Any) -> go.Figure:
    ...
    """
    # Load data files
    data_path = getattr(run_ctx, 'data_path', None) or getattr(run_ctx, 'out_dir', Path('.')) / 'data.csv'
    trades_path = getattr(run_ctx, 'trades_path', None) or getattr(run_ctx, 'out_dir', Path('.')) / 'trades.csv'
    events_path = getattr(run_ctx, 'events_path', None) or getattr(run_ctx, 'out_dir', Path('.')) / 'events.parquet'
    ...
    if Path(events_path).exists():
        try:
            ...
            pivot_events = events[events['type'] == 'PivotEvent'] if 'type' in events.columns else pd.DataFrame()
...
def build_static_chart(run_ctx: Any, output_path: Path | None = None) -> Path:
    ...
    try:
        ...
        raise ImportError("mplfinance required for static plotting. Install with: pip install mplfinance") from e
        ...
        try:
            ...
            if 'entry_price' in trades.columns:
                additional_plots.append(
                    mpf.make_addplot(trades['entry_price'], type='scatter', marker='^', color='lime', markersize=70)
                )
...
def display_chart_in_chatgpt(fig: go.Figure) -> None:
    ...
    if os.getenv('CHATGPT_ENV') == '1':
        try:
            from python_user_visible import display_plotly  # type: ignore
            display_plotly(fig)
        except ImportError:
            print("Warning: python_user_visible not available in this environment")
    else:
        print("Not in ChatGPT environment, use fig.show() or fig.write_html() to view chart")
...
def build_plotly_from_data(
    ...
    if events_df is not None and not events_df.empty:
        fvg_events = events_df[events_df['type'] == 'fvg'] if 'type' in events_df.columns else events_df
        
        for _, event in fvg_events.iterrows():
            color = 'rgba(0,255,0,0.3)' if event.get('direction') == 'bullish' else 'rgba(255,0,0,0.3)'
...
def build_static_chart_from_data(
    ...
    try:
        ...
        raise ImportError("mplfinance required for static charts. Install with: pip install mplfinance")
