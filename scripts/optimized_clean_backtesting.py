#!/usr/bin/env python3
"""
Optimized Simple Clean Backtesting
Reduces API calls by caching HTF data
"""
...
class OptimizedCleanBacktester:
    """
    Optimized clean backtesting with HTF data caching
    """
    ...
    def __init__(self):
        ...
        # Cache HTF data to avoid repeated API calls
        self.htf_cache = {}
    ...
    def backtest_optimized(self, symbol: str, ltf: str, start: str, end: str) -> Dict:
        """
        Optimized backtesting with HTF data pre-loaded
        """
        print(f"🚀 Optimized Clean Backtesting")
        ...
        try:
            ...
            return {"error": f"LTF data fetch failed: {e}"}
        
        # Pre-load HTF data for the entire period
        self._preload_htf_data(symbol, start, end)
    ...
    def _preload_htf_data(self, symbol: str, start: str, end: str):
        """
        Pre-load HTF data to avoid repeated API calls
        """
        print("📡 Pre-loading HTF data...")
        
        # Extend range to get enough HTF context
        start_dt = datetime.fromisoformat(start.replace('Z', '+00:00'))
        end_dt = datetime.fromisoformat(end.replace('Z', '+00:00'))
        
        # Get HTF data for extended period
        htf_start = start_dt - timedelta(days=30)
        htf_end = end_dt + timedelta(days=1)
        ...
        try:
            htf_result = self.service.detect_signals(
                ...
                signal_type="fvg_and_pivot",
                timeframe="4H",
                start=htf_start.isoformat() + "Z",
                end=htf_end.isoformat() + "Z"
            )
            ...
            self.htf_cache[symbol] = htf_df
            print(f"   📊 HTF Candles cached: {len(htf_candles)}")
            
        except Exception as e:
            print(f"   ❌ HTF data fetch failed: {e}")
            self.htf_cache[symbol] = pd.DataFrame()
    
    def _process_chronologically(self, ltf_candles: List[Dict], symbol: str) -> Dict:
        """
        Process candles chronologically with cached HTF data
        """
        ...
        for i, candle in df.iterrows():
            ...
            # Get HTF context up to current time from cache
            htf_context = self._get_htf_context_cached(current_time, symbol)
            
            # Detect FVGs in HTF context
            new_fvgs = self._detect_fvgs_in_context(htf_context, current_time)
            
            # Add new FVGs to detection list
            for fvg in new_fvgs:
                # Check if FVG is already detected
                existing = False
                for detected_fvg in fvgs_detected:
                    if (abs(detected_fvg['zone_low'] - fvg['zone_low']) < 0.01 and
                        abs(detected_fvg['zone_high'] - fvg['zone_high']) < 0.01):
                        existing = True
                        break
    ...
    def _get_htf_context_cached(self, current_time: datetime, symbol: str) -> List[Dict]:
        """
        Get HTF context from cached data
        """
        if symbol not in self.htf_cache or self.htf_cache[symbol].empty:
            return []
        
        htf_df = self.htf_cache[symbol]
        
        # Filter HTF data up to current time
        filtered_df = htf_df[htf_df['timestamp'] <= current_time]
        
        # Return last 100 candles or all if less
        recent_df = filtered_df.tail(100)
        ...
        for fvg in fvgs:
            if (candle['low'] <= fvg['zone_high'] and 
                ...
                if len(ltf_history) >= 50:  # Need history for EMAs
                    ...
                    recent = ltf_history.tail(5)
                    
                    for j in range(1, len(recent)):
                        curr = recent.iloc[j]
                        prev = recent.iloc[j-1]
                        
                        # Bullish crossover with bearish FVG
                        if (prev['ema_9'] <= prev['ema_20'] and 
                            curr['ema_9'] > curr['ema_20'] and
                        ...
                        # Bearish crossover with bullish FVG
...
def test_optimized_clean_backtesting():
    """Test optimized clean backtesting"""
    
    print("🚀 Testing Optimized Clean Backtesting")
    print("=" * 80)
    
    backtester = OptimizedCleanBacktester()
    
    try:
        # Test with actual valid historical data
        results = backtester.backtest_optimized(
            symbol="BTC/USD",
            ltf="15T",
        ...
        if results['signals']:
            ...
            for i, signal in enumerate(results['signals'][:5]):  # Show first 5
                ...
                print(f"      FVG: {signal['fvg_zone']}, EMA9: {signal['ema_9']:.2f}, EMA20: {signal['ema_20']:.2f}")
        
        if results['fvgs_detected']:
            print(f"\n📈 FVG Details:")
            for i, fvg in enumerate(results['fvgs_detected'][:3]):  # Show first 3
                print(f"   {i+1}. {fvg['timestamp']}: {fvg['direction']} FVG {fvg['zone_low']:.2f}-{fvg['zone_high']:.2f}")
    ...
    print(f"\n✅ Optimized backtesting test complete!")


if __name__ == "__main__":
    test_optimized_clean_backtesting()
