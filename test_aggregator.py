...
from core.strategy.timeframe import get_bucket_id, get_bucket_start, TimeframeConfig
...
class TestTimeframeUtils:
    ...
    def test_bucket_id_calculation(self):
        ...
        # Test H1 bucket IDs
        dt_1 = datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc)  # 10:00
        dt_2 = datetime(2024, 1, 1, 10, 30, 0, tzinfo=timezone.utc)  # 10:30
        dt_3 = datetime(2024, 1, 1, 11, 0, 0, tzinfo=timezone.utc)  # 11:00
...
class TestCandleBuffer:
    ...
    def test_buffer_basic_operations(self):
        ...
        o, h, l, c, v = buffer.get_ohlcv()
        assert o == 100.0  # First open
        assert h == 115.0  # Highest high
        assert l == 95.0   # Lowest low
        assert c == 110.0  # Last close
        assert v == 2500   # Sum of volumes
    ...
    def test_buffer_overflow(self):
        ...
        o, h, l, c, v = buffer.get_ohlcv()
        assert o == 105.0  # candle2 open
        assert c == 115.0  # candle3 close
        assert v == 3500   # candle2 + candle3 volumes
...
class TestTimeAggregator:
    ...
    def test_h1_aggregation_basic(self):
        ...
        assert h1_candle_1.ts == datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        ...
        assert h1_candle_2.ts == datetime(2024, 1, 1, 11, 0, 0, tzinfo=timezone.utc)
...
class TestMultiTimeframeAggregator:
    ...
    def test_multiple_timeframes(self):
        ...
        start_time = datetime(2024, 1, 1, 8, 0, 0, tzinfo=timezone.utc)  # Start at 8:00
...
class TestRealWorldScenarios:
    ...
    def test_market_hours_aggregation(self):
        ...
        start_time = datetime(2024, 1, 1, 14, 30, 0, tzinfo=timezone.utc)  # 9:30 AM ET
    ...
    def test_weekend_gap_handling(self):
        ...
        # Friday data
        friday = datetime(2024, 1, 5, 20, 0, 0, tzinfo=timezone.utc)  # Friday 8 PM
        friday_candles = []
        for i in range(60):
            candle = Candle(
        ...
        monday = datetime(2024, 1, 8, 1, 0, 0, tzinfo=timezone.utc)  # Monday 1 AM
        monday_candles = []
        for i in range(60):
            candle = Candle(
                ts=monday.replace(minute=i),
                open=102.0, high=103.0, low=101.0, close=102.5, volume=1500
            )
